...
public class TrackDAOImpl implements TrackDAO {
    ...
    public List<Track> getTracksNotInPlaylistSearch(String ownerName, String name, String search) {
        List<Track> tracks = new ArrayList<Track>();
        try{
            CallableStatement cs = this.conn.prepareCall("{call spd_getTracksNotInPlaylistSearch(?, ?, ?)}");
            cs.setString(1, ownerName);
            cs.setString(2, name);
    ...
    public List<Track> getTracks() {
        List<Track> tracks = new ArrayList<Track>();
        try{
            CallableStatement cs = this.conn.prepareCall("{call spd_getTracks()}");
            ResultSet rs = cs.executeQuery();
            while (rs.next()) {
                String performer = rs.getString("performer");
                String title = rs.getString("title");
    ...
    public List<Track> getTracksSearch(String search) {
        List<Track> tracks = new ArrayList<Track>();
        try{
            CallableStatement cs = this.conn.prepareCall("{call spd_getTracks(?)}");
